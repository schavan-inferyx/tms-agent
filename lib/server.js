...
const { getMachineId, launchChromeForRun, DATA_DIR } = require('./chrome');
const { bridgeCdp } = require('./bridge');
const { tmsFetch } = require('./tmsFetch');
const { probeAgentHealth } = require('./probe');
...
async function handleRun(runToken) {
  ...
  try {
    ...
    const profileDir = path.join(DATA_DIR, 'chrome-profiles', validated.runId);
    const launched = await launchChromeForRun(profileDir, validated.browserLaunch || null);
    chromeChild = launched.child;
    const debugPort = launched.debugPort;
    const chromeWsUrl = launched.wsUrl;
...
function startServer() {
  ...
  server.on('error', (err) => {
    ...
    probeAgentHealth(port).then((health) => {
      if (health?.version === INSTALLED_VERSION) {
        console.log(`tms-agent ${INSTALLED_VERSION} already running on http://127.0.0.1:${port}`);
        process.exit(0);
        return;
      }
      if (health?.version) {
        console.error(
          `Port ${port} is in use by tms-agent ${health.version}, but this binary is ${INSTALLED_VERSION}.`
          + ` Run: npm install -g tms-agent@${INSTALLED_VERSION}`,
        );
        process.exit(1);
        return;
      }
