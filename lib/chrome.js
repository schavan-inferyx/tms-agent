...
function chromeInstallHint() {
  if (process.platform === 'linux') {
    return 'Install Google Chrome: sudo apt install google-chrome-stable';
  }
...
function sanitizeChromeArgs(args) {
  ...
  const linuxBlocked = [/--start-maximized/i];
  return (Array.isArray(args) ? args : []).filter((arg) => {
    const value = String(arg);
    if (!value) return false;
    if (blocked.some((re) => re.test(value))) return false;
    if (process.platform === 'linux' && linuxBlocked.some((re) => re.test(value))) return false;
    return true;
  });
}
...
function clearStaleProfileLock(userDataDir) {
  for (const name of ['SingletonLock', 'SingletonSocket', 'SingletonCookie']) {
    ...
    try {
      ...
      /* profile may be in use */
    }
  }
}

function readLaunchLogTail(userDataDir, maxLines = 8) {
  const logPath = path.join(userDataDir, 'chrome-launch.log');
...
function assertChromeCanLaunch() {
  ...
  const err = new Error(
    'tms-agent is running as root — Chrome cannot start that way. '
    + 'Run: sudo fuser -k 9345/tcp && sudo npm uninstall -g tms-agent '
    + '&& npm install -g tms-agent (no sudo) && tms-agent &',
  );
  err.code = 'chrome_root_user';
  throw err;
}
...
function launchChrome(debugPort, userDataDir, browserLaunch = {}) {
  ...
  fs.writeSync(logFd, `\n--- launch ${new Date().toISOString()} port=${debugPort} ---\n`);
  fs.writeSync(logFd, `${exe} ${args.join(' ')}\n`);
...
async function waitForDebuggerUrl(port, chromeChild = null, userDataDir = '', attempts = 120) {
  ...
  const logHint = readLaunchLogTail(userDataDir);
  const displayHint = process.platform === 'linux' && !process.env.DISPLAY
    ? ' DISPLAY may be unset — start agent from your desktop terminal, not SSH without -X.'
    : '';
  const err = new Error(
    `Chrome debug port ${port} did not become ready in ${Math.round(attempts * 0.25)}s.`
    + `${displayHint} ${chromeInstallHint()}`
    + (logHint ? ` Log: ${logHint}` : ''),
  );
...
/** Try full Playwright args, then channel-only, then minimal Chrome flags. */
async function launchChromeForRun(profileDir, browserLaunch = {}) {
  const launchPlans = [
    browserLaunch,
  ...
  for (const plan of launchPlans) {
    ...
    try {
      const child = launchChrome(debugPort, profileDir, plan);
      const wsUrl = await waitForDebuggerUrl(debugPort, child, profileDir);
      return { child, debugPort, wsUrl };
    } catch (err) {
      lastErr = err;
      if (err.code !== 'chrome_debug_port_timeout' && err.code !== 'chrome_launch_failed') {
        throw err;
      }
    }
  }
