...
async function waitForDebuggerUrl(port, chromeChild = null, attempts = 60) {
  for (let i = 0; i < attempts; i += 1) {
    ...
    if (chromeChild && chromeChild.exitCode !== null) {
      const err = new Error(
        `Chrome exited before the debug port was ready (code ${chromeChild.exitCode}). `
        + 'Close other Chrome windows using the automation profile, or run: '
        + 'pkill -f "chrome-profile" then retry.',
      );
      err.code = 'chrome_launch_failed';
      throw err;
    }
